...
import { CheckCircle2, XCircle, BarChart, Target, TrendingUp, Check, Lightbulb, BrainCircuit, MessageSquareQuote, Medal, Search, FileText, Hash, Eye, ListChecks, Repeat, ChevronDown, ChevronUp } from "lucide-react";
...
const QueryAnalysisDetail = ({ result, domain }: { result: AnalysisItem; domain: string }) => {
  ...
  return (
    <div className="space-y-6 pt-2">
      ...
      <div className="space-y-4">
        ...
        <div className="prose prose-sm max-w-none text-muted-foreground bg-accent p-4 rounded-lg border border-primary/20">
            <ReactMarkdown>
                {isAnswerExpanded ? result.ai_answer : answerLines.slice(0, 4).join('\n')}
            </ReactMarkdown>
        </div>
        {showExpandButton && (
            <Button variant="link" onClick={() => setIsAnswerExpanded(!isAnswerExpanded)} className="text-primary">
            ...
            {result.sources.slice(0, 15).map((source, i) => (
              <a 
                ...
                className={cn(
                  ...
                  source.includes(domainToMatch) && "bg-primary/80 text-primary-foreground border-2 border-primary-foreground animate-pulse"
                )}
              >
                {source}
              </a>
...
export function AnalysisResults({ results, onReset }: AnalysisResultsProps) {
  if (!results || results.analysis.length === 0) {
    return (
        <Card className="w-full max-w-2xl mx-auto shadow-lg">
            <CardHeader>
                <CardTitle>Analysis Failed</CardTitle>
                <CardDescription>Something went wrong during the analysis. Please try again later.</CardDescription>
            </CardHeader>
            <CardFooter>
                <Button onClick={() => onReset()} size="lg" className="w-full bg-gradient-to-r from-primary to-secondary text-primary-foreground">Start Over</Button>
            </CardFooter>
        </Card>
    );
  }

  const totalRecommendations = results.analysis.reduce((acc, item) => acc + item.recommendations.length, 0);

  return (
    <div className="w-full space-y-12">
      <header className="text-center space-y-2">
        <h2 className="text-3xl sm:text-4xl font-bold text-foreground">Brand Visibility Analysis Complete</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard icon={Target} title="AI Search Visibility" value={results.ai_visibility} colorClass="--primary" />
        <MetricCard icon={TrendingUp} title="Average Ranking Position" value={results.average_rank === 0 ? "N/A" : `#${results.average_rank.toFixed(0)}`} colorClass="--chart-2" />
        <MetricCard icon={Check} title="Visibility Score" value={`${results.visibility_score.toFixed(0)}%`} colorClass="--chart-4" />
      ...
      <Card className="w-full shadow-2xl shadow-primary/5 border-2 border-primary/10">
        <CardHeader>
          ...
          <CardDescription className="text-base">
            Here's a breakdown of brand visibility for each search query. Click on a query to see the details.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Accordion type="single" collapsible className="w-full" defaultValue="item-0">
            {results.analysis.map((result, index) => (
              <AccordionItem value={`item-${index}`} key={index} className="border-b-2 border-primary/10">
                ...
                <AccordionContent className="bg-accent/50 p-6 rounded-b-lg">
                  <QueryAnalysisDetail result={result} domain={results.domain} />
                </AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
